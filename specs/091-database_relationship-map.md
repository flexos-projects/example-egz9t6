...
**DEPRECATION NOTICE:** This document defines relationships for a SQL-based schema. The project has moved to a Git-based content model using Astro Content Collections. The authoritative schema and its relationships are now defined in `builds/001/specs/006-src-content-config-ts.md` using `reference()`. This file is kept for historical reference only.
